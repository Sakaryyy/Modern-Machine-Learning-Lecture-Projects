...
class EnergyBudgetEnv(gym.Env):
    ...
    def __init__(self, config: EnvironmentConfig | None = None) -> None:
        ...
        self.observation_space = gym.spaces.Box(
            ...
            high=np.array([23.0, np.inf, self.config.max_battery_energy], dtype=np.float32),
            dtype=np.float32,
        )
    ...
    def reset(
        ...
        self._time_step = 0
        self._battery_energy = self.config.max_battery_energy // 2
        self._sample_exogenous(self._time_step % 24)
        observation = self._get_observation()
        info = {"battery_energy": self._battery_energy}
        self._logger.info("Environment reset: %s", info)
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        discharge_available = min(battery_to_demand, self._battery_energy, demand_remaining)
        battery_energy_after_discharge = self._battery_energy - discharge_available

        capacity_remaining = self.config.max_battery_energy - battery_energy_after_discharge
        ...
        grid_to_demand = max(0, demand_remaining - discharge_available)
        ...
        self._battery_energy = min(
            ...
            battery_energy_after_discharge + solar_to_battery + grid_to_battery,
        )
        ...
        metrics = StepMetrics(
            ...
            battery_to_demand=discharge_available,
        ...
        self._time_step += 1
        self._sample_exogenous(self._time_step % 24)
        terminated = self._time_step >= self.config.episode_length
        truncated = False

        observation = self._get_observation()
        info = {"metrics": metrics}
        return observation, float(balance), terminated, truncated, info
    ...
    def _solar_intensity(self, time_of_day: int) -> float:
        ...
        cloud_cover = self._rng.uniform(0.0, 1.0)
        solar_angle = 2 * np.pi * time_of_day / 24
        intensity = max(0.0, -cloud_cover * np.cos(solar_angle))
    ...
    def _sample_exogenous(self, time_of_day: int) -> None:
        ...
        self._current_market_price = self._market_price(
            time_of_day, self._current_solar_intensity
        )
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        time_of_day = self._time_step % 24
        buying_price = self.config.base_price + self._current_market_price
        observation = np.array(
            [time_of_day, buying_price, self._battery_energy],
            dtype=np.float32,
        )
        return observation
